...
parser.add_option("-o", "--output", dest="outfile",
                  help="Specify output xml file name, default path: output.xml", metavar="FILE", default="output.xml")
...
#Initiate logger
logging.basicConfig(filename='vp_generator.log', format='%(asctime)s %(levelname)s:%(message)s', datefmt='%m/%d/%Y %I:%M:%S %p', level=logging.DEBUG)
logging.info('Starting vp_generator...')
...
if __name__ == '__main__':
    ...
    tree = gen_xml_tree_root()
    logging.info('Building XML header...')
    if options.mode == "normal" or options.mode == "temporal":
        mode=options.mode
    else:
        mode="normal"
        logging.error("Invalid parameter mode, please enter accepted value : normal or temporal")
    gen_xml_tree_header(tree, mode)
    # Generate the column header and return root tag <Columns>
    logging.info('Building XML columns header...')
    col_root = gen_column_header(tree)
